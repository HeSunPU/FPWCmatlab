...
for k_EM = 1 : n_systemID
    ...
    for itr = 1 : Nitr
        ...
        if itr == 1
            ...
            dImeasured = mean(data.I(:,:,:,itr) - data.I(:,:,:,itr - 1),3);
        end
        ...
        dEmodel = zeros(darkHole.pixelNum,target.broadSampleNum);
        switch controller.whichDM
            case '1'
                for iE = 1:1:target.broadSampleNum
                    dEmodel(:,iE) = model.G1(:,:,iE) * command;
                end
            case '2'
                for iE = 1:1:target.broadSampleNum
                    dEmodel(:,iE) = model.G2(:,:,iE) * command;
                end
            case 'both'
                for iE = 1:1:target.broadSampleNum
                    dEmodel(:,iE) = model.G1(:,:,iE) * command(1:DM.activeActNum) + model.G2(:,:,iE) * command(DM.activeActNum + 1 : end);
                end
            otherwise
                disp('You can only use the first DM, second DM or both for wavefront control.');
                return;
        end
        EfocalEstNewBB = EfocalEstBroadband + dEmodel;
        dImodel = mean(abs(EfocalEstNewBB).^2 - abs(EfocalEstBroadband).^2,2);
